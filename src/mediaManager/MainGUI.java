 ...
 *
 * The UI has a two-panel layout: a fixed Artists list on the left and a content
 * pane on the right. Navigation depth is derived from two main fields:
 * {@code selectedArtist} and {@code selectedAlbum}.
 *
 * Each panel owns its own "+" button. The left "+" button always adds an
 * {@link Artist}. The right "+" is context-dependent: it adds an {@link Album}
 * when an artist is selected and adds a {@link Song} when an album is selected.
 * When nothing is selected (all-songs view), the right "+" is disabled because
 * adding a song requires an album context.
 * 
 * File operations are delegated to {@link FileHandler}. The current file is
 * tracked by currentFile, if set. If not set, it performs in-memory writes.
 * Note: File operations are not fully implemented.
 *
...
public class MainGUI extends JFrame {
	 ...
	 * Version 1.0
	 */
	private static final long serialVersionUID = 8593422972402521583L;
	...
	private final DefaultListModel<Object> contentModel = new DefaultListModel<>(); // this is Object because it must
																					// hold either Album or Song
	...
	private void dialogBoxFormatting() {
		...
		UIManager.put("OptionPane.border", BorderFactory.createLineBorder(new Color(192, 192, 192)));

		UIManager.put("OptionPane.messageAreaBorder", BorderFactory.createEmptyBorder(10, 10, 10, 10));

		UIManager.put("OptionPane.buttonAreaBorder", BorderFactory.createEmptyBorder(10, 10, 10, 10));
	...
	private JPanel buildSearchBar() {
		...
		searchField.setBorder(
				new CompoundBorder(new LineBorder(new Color(192, 192, 192), 1, true), new EmptyBorder(0, 5, 0, 0)));
	...
	private JPanel buildArtistPanel() {
		JPanel panel = new JPanel(new BorderLayout(4, 4));
		panel.setBackground(new Color(23, 25, 32));
		panel.setBorder(new CompoundBorder(new EmptyBorder(0, 10, 0, 0),
				new CompoundBorder(new LineBorder(new Color(192, 192, 192), 1, true), new EmptyBorder(5, 5, 5, 5))));
		...
		addArtistButton.setBorder(
				new CompoundBorder(new LineBorder(new Color(192, 192, 192), 1, true), new EmptyBorder(6, 8, 6, 8)));
	...
	private JPanel buildContentPanel() {
		JPanel panel = new JPanel(new BorderLayout(4, 4));
		panel.setBackground(new Color(23, 25, 32));
		panel.setBorder(new CompoundBorder(new EmptyBorder(0, 0, 0, 10),
				new CompoundBorder(new LineBorder(new Color(192, 192, 192), 1, true), new EmptyBorder(5, 5, 5, 5))));
	...
	private void onRemoveArtist() {
		...
		int confirm = JOptionPane.showConfirmDialog(this, "Delete artist \"" + selected.getName() + "\"?",
	...
	private void onRemoveAlbum() {
		...
		int confirm = JOptionPane.showConfirmDialog(this, "Delete album \"" + selectedAlbum.getName() + "\"?",
				"Confirm Delete", JOptionPane.YES_NO_OPTION);
	...
	private void onRemoveSong() {
		...
		int confirm = JOptionPane.showConfirmDialog(this, "Delete song \"" + song.getTitle() + "\"?", "Confirm Delete",
	...
	private void onAddArtist() {
		...
		String name = JOptionPane.showInputDialog(this, "Artist name:", "Add Artist", JOptionPane.PLAIN_MESSAGE);
		if (name == null) {
			System.out.println("[GUI] Cancelled add operation or empty input");
			return;
		}
		if (name.isBlank()) {
			JOptionPane.showMessageDialog(this, "Artist name is required.", "Missing field",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		if (name.contains("|")) {
			JOptionPane.showMessageDialog(this, "Names cannot contain the '|' character.", "Invalid name",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
	...
	private void showAddAlbumDialog(Artist parent) {
		String name = JOptionPane.showInputDialog(this, "Album name:", "Add Album", JOptionPane.PLAIN_MESSAGE);
		...
		if (name.isBlank()) {
			JOptionPane.showMessageDialog(this, "Album name is required.", "Missing field",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		if (name.contains("|")) {
		...
		System.out.println("[GUI] Added album '" + album.getName() + "' to artist '" + parent.getName() + "'");
		autoSave();
		refreshContentPane();
	}
	...
	private void showAddSongDialog(Album parent) {
		...
		Object[] form = { "Title:", titleField, "Duration (M:SS):", durationField };
		int result = JOptionPane.showConfirmDialog(this, form, "Add Song", JOptionPane.OK_CANCEL_OPTION,
		...
		if (title.isBlank() || durationStr.isBlank()) {
			JOptionPane.showMessageDialog(this, "Title and duration are required.", "Missing field",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		if (title.contains("|")) {
			JOptionPane.showMessageDialog(this, "Title cannot contain the '|' character.", "Invalid title",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		...
		if (duration == null || duration <= 0) {
			JOptionPane.showMessageDialog(this, "Duration must be in M:SS format (such as 3:45) or a positive decimal.",
	 ...
	 * decimal ("3.45"). Returns null if the string is malformed. The stored
	 * encoding matches Song.toString: 3.45 represents 3 minutes 45 seconds.
	 */
	...
	private void showDuplicateWarning(String message) {
		JOptionPane.showMessageDialog(this, message, "Duplicate", JOptionPane.WARNING_MESSAGE);
	}
			...
			for (Artist artist : library.getArtists()) {
				for (Album album : artist.getAlbums()) {
					for (Song song : album.getSongs()) {
						...
						String text = String.format("%-15s %-15s %-15s %-8s", truncate(artist.getName(), 13),
								truncate(album.getName(), 13), truncate(song.getTitle(), 15),
	...
	private void onOpen() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Failed to open file: " + e.getMessage(), "Open error",
	...
	private void onSave() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Failed to save file: " + e.getMessage(), "Save error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	/**
	 * Auto Save is a function that will for a save with any new addition to Media
	 * Collection Manager.
	 */
	private void autoSave() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Auto-save failed: " + e.getMessage(), "Save error",
					JOptionPane.ERROR_MESSAGE);
